 ...
 */
static inline bool
use_decompress_chunk_node(const RelOptInfo *rel, const RangeTblEntry *rte, const Hypertable *ht,
						  const Chunk *chunk)
{
	/*
	 ...
	 */
	return ts_should_use_transparent_decompression(ht, chunk->amoid) &&
						   ...
						   Hypertable *ht)
{
	...
	if (use_decompress_chunk_node(rel, rte, ht, chunk))
	{
